...
# You no longer need to hardcode API keys or service account paths here.
# They are accessed via st.secrets.
...
if not firebase_admin._apps:
    try:
        # Load service account key directly from Streamlit secrets (JSON string)
        # It's important to parse the string back into a dictionary
        if "FIREBASE_SERVICE_ACCOUNT_KEY" not in st.secrets:
            st.error("Firebase service account key not found in Streamlit secrets. Please configure it.")
            st.stop()
        
        # Parse the JSON string from secrets into a dictionary
        FIREBASE_SERVICE_ACCOUNT_CONFIG = json.loads(st.secrets["FIREBASE_SERVICE_ACCOUNT_KEY"])
        ...
        print("DEBUG: Firebase initialized successfully in terminal.")
        print(f"DEBUG: Initialized with Storage Bucket: {st.session_state['bucket'].name}")
    except Exception as e:
        st.error(f"Error initializing Firebase: {e}.")
        st.error("Please ensure your Streamlit secrets are correctly configured for Firebase.")
...
def get_comparative_ai_analysis(jd_text, all_cv_data):
    ...
    Your output MUST be a single JSON object with the following structure, containing two distinct arrays for tables and two strings for text sections:
    {
      ...
      "additional_observations_text": "...", 
      "final_shortlist_recommendation": "..." 
    }
