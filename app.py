...
import time # Added for debugging message visibility

# --- Firebase Imports ---
import firebase_admin
from firebase_admin import credentials, auth, firestore, storage
from firebase_admin import exceptions # Import exceptions module for FirebaseError
import requests # Keep requests for potential other external calls if needed

# --- AI & Document Processing Imports ---
from openai import OpenAI
import pandas as pd
from PyPDF2 import PdfReader # Corrected import from PyPDF2
from docx import Document
from docx.shared import Inches, Pt
from docx.enum.text import WD_ALIGN_PARAGRAPH
from docx.enum.section import WD_SECTION_START
from docx.enum.table import WD_ALIGN_VERTICAL # Used for table alignment

# --- Streamlit Page Configuration (MUST BE THE FIRST ST COMMAND) ---
# This must be called at the very beginning of the script.
st.set_page_config(
...
# --- Configuration ---
# IMPORTANT: These keys and bucket name are specific to your project.
# For security, consider using Streamlit Secrets for deployment instead of hardcoding.

# Firebase Web API Key (obtained from Firebase Console -> Project settings -> Your apps -> Web app config)
# Keeping this for reference, but login will now use Admin SDK
FIREBASE_WEB_API_KEY = "AIzaSyB3Db21t5Djc5O2ToU_KF5WCBmWMgM" # Keep your LATEST key here

# OpenAI API Key (the one you just provided)
OPENAI_API_KEY = "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" # <-- YOUR CORRECT OPENAI KEY

# Path to your Firebase Admin SDK Service Account Key JSON file.
# This file MUST be named 'serviceAccountKey.json' and be in the same directory as app.py
FIREBASE_ADMIN_SDK_PATH = "serviceAccountKey.json"

# Firebase Storage Bucket Name (from Firebase Console -> Storage)
# This MUST match the project ID where you set up Storage ('ecr-app-drive-integration')
FIREBASE_STORAGE_BUCKET_NAME = 'ecr-app-drive-integration.appspot.com' # <-- YOUR CORRECT STORAGE BUCKET NAME
...
if not firebase_admin._apps:
    try:
        ...
        # Get Firestore and Storage bucket clients and store them in session state
        st.session_state['db'] = firestore.client()
        # Explicitly specify the bucket name when getting the bucket object
        st.session_state['bucket'] = storage.bucket(FIREBASE_STORAGE_BUCKET_NAME)
        st.success("Firebase initialized successfully!")
        print("DEBUG: Firebase initialized successfully in terminal.") # Added terminal print
    except Exception as e:
        # Provide detailed error message for troubleshooting Firebase initialization issues
        st.error(f"Error initializing Firebase: {e}.")
        st.error("Please ensure:")
        st.error(f"1. The file '{FIREBASE_ADMIN_SDK_PATH}' exists in the same directory as app.py.")
        st.error(f"2. The content of '{FIREBASE_ADMIN_SDK_PATH}' is the correct JSON for your Firebase project ID 'ecr-app-drive-integration'.")
        st.error(f"3. Your 'FIREBASE_STORAGE_BUCKET_NAME' in the code matches '{FIREBASE_STORAGE_BUCKET_NAME}'.")
        print(f"ERROR: Firebase initialization failed: {e}") # Added terminal print
...
try:
    ...
    st.error(f"OpenAI client not initialized: {e}. Please check your OPENAI_API_KEY.")
    print(f"ERROR: OpenAI client initialization failed: {e}") # Added terminal print
...
def extract_text_from_pdf(uploaded_file_bytes_io):
    ...
    try:
        ...
        print(f"ERROR (extract_text_from_pdf): {e}") # Added terminal print
        return None
...
def extract_text_from_docx(uploaded_file_bytes_io):
    ...
    try:
        ...
        print(f"ERROR (extract_text_from_docx): {e}") # Added terminal print
        return None
...
def admin_user_management_page():
    ...
    try:
        ...
        if users_data:
            ...
            # Forms for user actions (toggle admin, delete)
    ...
    with st.form("update_password_form"):
        ...
        if submit_update_button:
            ...
            try:
                ...
                print(f"ERROR (update_password_page): Unexpected Python Error during password update: {e}") # Added terminal print
